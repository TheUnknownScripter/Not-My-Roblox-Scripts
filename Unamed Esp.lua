			...
			if typeof(FirstName) == "string" and #FirstName > 0 then
				...
				if not IsStringEmpty(Player:GetAttribute("HouseRank")) then
					Prefix = Player:GetAttribute("HouseRank") == "Owner"
							and (Player:GetAttribute("Gender") == "Female" and "Lady " or "Lord ")
						or ""
				end
				...
				if Character then
					...
					if
						Character:FindFirstChild("ManaAbilities")
						and Character.ManaAbilities:FindFirstChild("ManaSprint")
					then
	...
	[3541987450] = {
		CustomPlayerTag = function(Player)
			...
			if Player:FindFirstChild("leaderstats") then
				...
				local Extra = {}
				if
					Player.leaderstats:FindFirstChild("Prestige")
					and Player.leaderstats.Prestige.ClassName == "IntValue"
					and Player.leaderstats.Prestige.Value > 0
				then
					Name = Name .. "#" .. tostring(Player.leaderstats.Prestige.Value) .. " "
				end
				if
					Player.leaderstats:FindFirstChild("HouseRank")
					and Player.leaderstats:FindFirstChild("Gender")
					and Player.leaderstats.HouseRank.ClassName == "StringValue"
					and not IsStringEmpty(Player.leaderstats.HouseRank.Value)
				then
					Prefix = Player.leaderstats.HouseRank.Value == "Owner"
							and (Player.leaderstats.Gender.Value == "Female" and "Lady " or "Lord ")
						or ""
				end
				if
					Player.leaderstats:FindFirstChild("FirstName")
					and Player.leaderstats.FirstName.ClassName == "StringValue"
					and not IsStringEmpty(Player.leaderstats.FirstName.Value)
				then
					Name = Name .. "" .. Prefix .. Player.leaderstats.FirstName.Value
				end
				if
					Player.leaderstats:FindFirstChild("LastName")
					and Player.leaderstats.LastName.ClassName == "StringValue"
					and not IsStringEmpty(Player.leaderstats.LastName.Value)
				then
					Name = Name .. " " .. Player.leaderstats.LastName.Value
				end
				if
					Player.leaderstats:FindFirstChild("UberTitle")
					and Player.leaderstats.UberTitle.ClassName == "StringValue"
					and not IsStringEmpty(Player.leaderstats.UberTitle.Value)
				then
...
function MouseHoveringOver(Values)
	...
	return (MLocation.x >= X1 and MLocation.x <= (X1 + (X2 - X1)))
		and (MLocation.y >= Y1 and MLocation.y <= (Y1 + (Y2 - Y1)))
end
...
function CalculateValue(Min, Max, Percent)
	return Min + math.floor(((Max - Min) * Percent) + 0.5)
end
	...
	if _ then -- extremely ugly code yea i know but i dont care p.s. i hate pcall
		...
		if _ and typeof(Table) == "table" then
			for i, v in pairs(Table) do
				if
					typeof(Options[i]) == "table"
					and Options[i].Value ~= nil
					and (typeof(Options[i].Value) == "boolean" or typeof(Options[i].Value) == "number")
				then
...
Options("ResetSettings", "Reset Settings", function()
	for i, v in pairs(Options) do
		if
			Options[i] ~= nil
			and Options[i].Value ~= nil
			and Options[i].Text ~= nil
			and (
				typeof(Options[i].Value) == "boolean"
				or typeof(Options[i].Value) == "number"
				or typeof(Options[i].Value) == "EnumItem"
			)
		then
...
local function IsMouseOverDrawing(Drawing, MousePosition)
	...
	return MousePosition.X > TopLeft.X
		and MousePosition.Y > TopLeft.Y
		and MousePosition.X < BottomRight.X
		and MousePosition.Y < BottomRight.Y
end
	...
	function Picker:HandleInput(Input, P, Type)
		if Type == "Began" then
			if Input.UserInputType.Name == "MouseButton1" then
				...
				if
					MousePosition.X > SquareSV.Position.X
					and MousePosition.Y > SquareSV.Position.Y
					and MousePosition.X < SquareSV.Position.X + SquareSV.Size.X
					and MousePosition.Y < SquareSV.Position.Y + SquareSV.Size.Y
				then
					self.Item = "HL"
				end
...
function CreateMenu(NewPosition) -- Create Menu
	...
	local BasePosition = NewPosition
		or V2New(Camera.ViewportSize.X / 8 - (BaseSize.X / 2), Camera.ViewportSize.Y / 2 - (BaseSize.Y / 2))
	...
	delay(0.025, function() -- since zindex doesnt exist
	...
	GetTableData(Options)(
		function(i, v) -- just to make sure certain things are drawn before or after others, too lazy to actually sort table
			if typeof(v.Value) == "function" then
				local BaseSize = V2New(BaseSize.X, 30)
				local BasePosition = shared.MenuDrawingData.Instances.Filling.Position
					+ V2New(0, CPos + (25 * v.AllArgs[4]) - 35)
...
shared.UESP_InputEndedCon = UserInputService.InputEnded:Connect(function(input)
	...
	elseif input.UserInputType.Name == "Keyboard" then
		if Binding then
			...
			Binding = false
		elseif
			input.KeyCode == Options.MenuKey.Value
			or (input.KeyCode == Enum.KeyCode.Home and UserInputService:IsKeyDown(Enum.KeyCode.LeftControl))
		then
			Options.MenuOpen()
	...
	if (tick() - LastRefresh) > (Options.RefreshRate.Value / 1000) then
		...
		for i, v in pairs(RenderList.Instances) do
			if v.Instance ~= nil and v.Instance.Parent ~= nil and v.Instance:IsA("BasePart") then
				...
				Data.Instances["Tracer"] = Data.Instances["Tracer"]
					or NewDrawing("Line")({
						Transparency = 1,
						Thickness = 2,
					})
				Data.Instances["NameTag"] = Data.Instances["NameTag"]
					or NewDrawing("Text")({
				...
				Data.Instances["DistanceTag"] = Data.Instances["DistanceTag"]
					or NewDrawing("Text")({
				...
				if Pass then
					...
					if ScreenPosition.Z < 0 then
						...
						OPos = CFrame.Angles(0, 0, AT):vectorToWorldSpace(
					...
					if ScreenPosition.Z > 0 then
						...
						if Options.ShowDistance.Value or Options.ShowHealth.Value then
							...
							DistanceTag.Position = V2New(ScreenPositionUpper.X, ScreenPositionUpper.Y)
								+ V2New(0, NameTag.TextBounds.Y)
						else
		...
		for i, v in pairs(Players:GetPlayers()) do
			...
			Data.Instances["HeadDot"] = Data.Instances["HeadDot"]
				or NewDrawing("Circle")({
					Filled = true,
					NumSides = 30,
				})
			Data.Instances["NameTag"] = Data.Instances["NameTag"]
			...
			Data.Instances["DistanceHealthTag"] = Data.Instances["DistanceHealthTag"]
				or NewDrawing("Text")({
					Size = Options.TextSize.Value - 1,
					Center = true,
					Outline = Options.TextOutline.Value,
			...
			if Pass and Character then
				...
				if Character ~= nil and Head and HumanoidRootPart and not Dead then
					...
					local Color = Rainbow and Color3.fromHSV(tick() * 128 % 255 / 255, 1, 1)
						or (CheckTeam(v) and TeamColor or EnemyColor)
					...
					if Options.ShowTracers.Value then
						if
							TracerPosition.X >= Camera.ViewportSize.X
							or TracerPosition.Y >= Camera.ViewportSize.Y
							or TracerPosition.X < 0
							or TracerPosition.Y < 0
						then
							TracerPosition = V2New(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y - 135)
						end
					...
					if ScreenPosition.Z > 0 then
						local ScreenPositionUpper = WorldToViewport(
							(HumanoidRootPart:GetRenderCFrame() * CFrame.new(
								0,
								Head.Size.Y + HumanoidRootPart.Size.Y + (Options.YOffset.Value / 25),
								0
							)).Position
						)
						...
						if Options.ShowDistance.Value or Options.ShowHealth.Value then
							...
							DistanceTag.Position = (
								NameTag.Visible and NameTag.Position + V2New(0, NameTag.TextBounds.Y)
								or V2New(ScreenPositionUpper.X, ScreenPositionUpper.Y)
							)
						else
							DistanceTag.Visible = false
						end
...
local function Update()
	...
	if Options.MenuOpen.Value and MenuLoaded then
		...
		if MouseHeld then
			...
			if Dragging then
				DraggingWhat.Slider.Position = V2New(
					math.clamp(
				...
				local Percent = (DraggingWhat.Slider.Position.X - DraggingWhat.Line.Position.X)
					/ (
